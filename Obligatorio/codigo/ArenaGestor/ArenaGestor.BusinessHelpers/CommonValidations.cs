...
namespace ArenaGestor.BusinessHelpers
{
    public static class CommonValidations
    {
        public static void ValidId(int id)
        {
            if (id < 1)
            {
                throw new ArgumentException("The identifier must be higher than 0");
            }
        }

        public static bool ValidRequiredString(string requiredString)
        {
            return !string.IsNullOrWhiteSpace(requiredString) && requiredString.Length > 0;
        }
        ...
        public static bool ValidPassword(string password)
        {
            ...
            Regex regex = new Regex(@"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^\da-zA-Z]).{8,30}$");
